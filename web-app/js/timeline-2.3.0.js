...
var loadMe=function(){
	...
	try{
		/* Do not load css via ajax - will load seperatly!
		(function(){
...
loaded:false,loadingScriptsCount:0,error:null,params:{
bundle:"true"}};
...
if(i>=0){
return url}}node=node.nextSibling}}return null};
...
if(onerror){
try{
script.innerHTML=onerror}catch(e){
}script.setAttribute("onerror",onerror)}if(charset){
script.setAttribute("charset",charset)}script.type="text/javascript";
...
SimileAjax.Platform.os={
	isMac:false,isWin:false,isWin32:false,isUnix:false
};
SimileAjax.Platform.browser={
	isIE:false,isNetscape:false,isMozilla:false,isFirefox:false,isOpera:false,isSafari:false,majorVersion:0,minorVersion:0
};
(function(){
...
if(D>=0){
E(A.substring(D+8,B(A," ",D)))}}if(SimileAjax.Platform.browser.isIE){
var D=A.indexOf("msie ");
if(D>=0){
E(A.substring(D+5,B(A,";",D)))}}if(SimileAjax.Platform.browser.isNetscape){
var D=A.indexOf("rv:");
if(D>=0){
E(A.substring(D+3,B(A,")",D)))}}if(SimileAjax.Platform.browser.isFirefox){
var D=A.indexOf("firefox/");
if(D>=0){
E(A.substring(D+8,B(A," ",D)))}}if(!("localeCompare" in String.prototype)){
...
if(C in E){
try{
E[C].apply(E,B)}catch(F){
SimileAjax.Debug.exception("Error firing event of name "+C,F)}}else{
if(this._wildcardHandlerName!=null&&this._wildcardHandlerName in E){
try{
E[this._wildcardHandlerName].apply(E,[C])}catch(F){
...
if(!this._indexed){
this._index()}return new SimileAjax.EventIndex._Iterator(this._events,A,B,this._unit)};
SimileAjax.EventIndex.prototype.getReverseIterator=function(A,B){
if(!this._indexed){
this._index()}return new SimileAjax.EventIndex._ReverseIterator(this._events,A,B,this._unit)};
...
if(A==null){
return null}if(!this._indexed){
this._index()}var C=A._earliestOverlapIndex;
var B=this._events.elementAt(C).getEnd();
for(var D=C+1;
D<this._events.length();
D++){
B=this._unit.later(B,this._events.elementAt(D).getEnd())}return B};
...
D._earliestOverlapIndex=F}var G=1;
for(var F=0;
F<E;
F++){
var D=this._events.elementAt(F);
...
this._currentIndex=this._events.elementAt(this._currentIndex-1)._earliestOverlapIndex}this._currentIndex--;
...
this._findNext();
return A}else{
return null}},_findNext:function(){
var B=this._unit;
...
this._minIndex=this._events.elementAt(this._minIndex-1)._earliestOverlapIndex}this._maxIndex=A.find(function(E){
return B.compare(E.getStart(),D)});
...
hasNext:function(){
return this._index<this._events.length()},next:function(){
...
if(!I){
throw new Error("Invalid date string: "+C)}var B=(I[1]=="-")?-1:1;
var J=B*I[2];
var H=I[5];
var D=I[7];
var F=I[9];
...
if(K<=4){
G.setUTCDate(E+1-K)}else{
G.setUTCDate(E+8-K)}}else{
if(H){
G.setUTCDate(1);
G.setUTCMonth(H-1)}if(D){
G.setUTCDate(D)}}}return G};
...
D*=((C[2]=="-")?1:-1)}E[1]=E[1].substr(0,E[1].length-C[0].length)}SimileAjax.DateTime.setIso8601Time(B,E[1])}if(D==null){
D=B.getTimezoneOffset()}B.setTime(B.getTime()+D*60000);
return B};
SimileAjax.DateTime.parseIso8601DateTime=function(A){
try{
return SimileAjax.DateTime.setIso8601(new Date(0),A)}catch(B){
return null}};
SimileAjax.DateTime.parseGregorianDateTime=function(F){
if(F==null){
return null}else{
if(F instanceof Date){
return F}}var B=F.toString();
...
if(G.toLowerCase()=="bc"){
A=1-A}}else{
var A=parseInt(B)}var E=new Date(0);
E.setUTCFullYear(A);
return E}try{
return new Date(Date.parse(B))}catch(D){
return null}};
...
alert(C)}}}SimileAjax.Debug.log=A;
A(B)};
...
A=function(C){
if(!SimileAjax.Debug.silent){
alert(C)}}}SimileAjax.Debug.warn=A;
A(B)};
...
A=function(F,E){
if(E!=null){
console.error(E+" %o",F)}else{
console.error(F)}throw (F)}}else{
A=function(F,E){
if(!SimileAjax.Debug.silent){
alert("Caught exception: "+E+"\n\nDetails: "+("description" in F?F.description:F))}throw (F)}}}SimileAjax.Debug.exception=A;
A(B,D)};*/
...
if(F){
F=(F.nodeType==1||F.nodeType==9)?F:F.parentNode}return D(C,E,F)}return true};
if(SimileAjax.Platform.browser.isIE){
C.attachEvent("on"+B,A)}else{
...
E+=C.offsetLeft;
D+=C.offsetTop;
C=C.offsetParent}var A=document.body;
while(B!=null&&B!=A){
if("scrollLeft" in B){
E-=B.scrollLeft;
D-=B.scrollTop}B=B.parentNode}return{
left:E,top:D}};
SimileAjax.DOM.getSize=function(B){
var A=this.getStyle(B,"width");
var C=this.getStyle(B,"height");
if(A.indexOf("px")>-1){
A=A.replace("px","")}if(C.indexOf("px")>-1){
C=C.replace("px","")}return{
w:A,h:C}};
SimileAjax.DOM.getStyle=function(B,A){
if(B.currentStyle){
var C=B.currentStyle[A]}else{
if(window.getComputedStyle){
var C=document.defaultView.getComputedStyle(B,null).getPropertyValue(A)}else{
var C=""}}return C};
...
return{
x:B.clientX-A.left,y:B.clientY-A.top}}else{
return{
x:B.offsetX,y:B.offsetY}}}else{
var A=SimileAjax.DOM.getPageCoordinates(C);
if((B.type=="DOMMouseScroll")&&SimileAjax.Platform.browser.isFirefox&&(SimileAjax.Platform.browser.majorVersion==2)){
return{
x:B.screenX-A.left,y:B.screenY-A.top}}else{
return{
x:B.pageX-A.left,y:B.pageY-A.top}}}};
SimileAjax.DOM.getEventPageCoordinates=function(A){
if(SimileAjax.Platform.browser.isIE){
return{
x:A.clientX+document.body.scrollLeft,y:A.clientY+document.body.scrollTop}}else{
return{
x:A.pageX,y:A.pageY}}};
...
F++){
if(H==A[F]){
continue outer}}if(H.offsetWidth==0&&H.offsetHeight==0){
...
J+=D.offsetTop;
E+=D.offsetLeft;
D=D.offsetParent}if(E<=L&&J<=K&&(L-E)<H.offsetWidth&&(K-J)<H.offsetHeight){
return SimileAjax.DOM._hittest(H,L,K,A)}else{
...
if(D!=null){
D.appendChild(C)}return C}else{
var A=null;
if("tag" in F){
var J=F.tag;
if(D!=null){
if(J=="tr"){
A=D.insertRow(D.rows.length)}else{
if(J=="td"){
A=D.insertCell(D.cells.length)}}}if(A==null){
A=J=="input"?SimileAjax.DOM.createInputElement(F.type):document.createElement(J);
if(D!=null){
D.appendChild(A)}}}else{
A=F.elmt;
if(D!=null){
D.appendChild(A)}}for(var B in F){
var H=F[B];
if(B=="field"){
G[H]=A}else{
if(B=="className"){
...
var I=H[n];
if(n=="float"){
n=SimileAjax.Platform.browser.isIE?"styleFloat":"cssFloat"}A.style[n]=I}}else{
if(B=="children"){
for(var E=0;
E<H.length;
E++){
SimileAjax.DOM._createDOMFromTemplate(H[E],G,A)}}else{
if(B!="tag"&&B!="elmt"){
A.setAttribute(B,H)}}}}}}}}}return A}}};
SimileAjax.DOM._cachedParent=null;
SimileAjax.DOM.createElementFromString=function(A){
if(SimileAjax.DOM._cachedParent==null){
SimileAjax.DOM._cachedParent=document.createElement("div")}SimileAjax.DOM._cachedParent.innerHTML=A;
...
var A=C.nextSibling;
if(C.nodeType==1){
SimileAjax.DOM._processDOMConstructedFromString(E,C,D)}C=A}};
SimileAjax.Graphics=new Object();
SimileAjax.Graphics.pngIsTranslucent=(!SimileAjax.Platform.browser.isIE)||(SimileAjax.Platform.browser.majorVersion>6);
if(!SimileAjax.Graphics.pngIsTranslucent){
SimileAjax.includeCssFile(document,SimileAjax.urlPrefix+"styles/graphics-ie6.css")}SimileAjax.Graphics._createTranslucentImage1=function(A,C){
var B=document.createElement("img");
B.setAttribute("src",A);
if(C!=null){
B.style.verticalAlign=C}return B};
...
if(SimileAjax.Platform.browser.isIE){
B.style.filter="progid:DXImageTransform.Microsoft.Alpha(Style=0,Opacity="+A+")"}else{
...
if(typeof B!="number"){
B=300}if(typeof A!="number"){
A=0}F.style.position="absolute";
F.style.left="-5000px";
F.style.top="0px";
F.style.width=B+"px";
...
if(A>0&&J>A){
J=A;
G=H-25}var I=SimileAjax.Graphics.createBubbleForPoint(E,C,H,J,D);
...
K.style.width=G+"px";
K.appendChild(F);
I.content.appendChild(K)}else{
F.style.width=H+"px";
I.content.appendChild(F)}},200)};
...
H.style.top=W+"px";
H.style.left=(B-E.arrowGraphicTargetOffset-J)+"px"}else{
var S=Z("left");
S.style.top=(A-V-W)+"px";
H.style.top=W+"px";
...
try{
this.f(D,D-this.current)}catch(C){
}this.current=D;
...
G.style.margin="2px";
if(SimileAjax.Graphics.pngIsTranslucent){
G.style.background="url("+F+") no-repeat"}else{
G.style.filter="progid:DXImageTransform.Microsoft.AlphaImageLoader(src='"+F+"', sizingMethod='image')"}var A;
if(SimileAjax.Platform.browser.isIE){
A="filter:alpha(opacity=0)"}else{
A="opacity: 0"}G.innerHTML="<textarea rows='1' autocomplete='off' value='none' style='"+A+"' />";
var C=G.firstChild;
C.style.width=B+"px";
C.style.height=D+"px";
...
A=C.offsetWidth;
B=C.offsetHeight}else{
var D=C.getBoundingClientRect();
A=Math.ceil(D.right-D.left);
B=Math.ceil(D.bottom-D.top)}return{
width:A,height:B}};
...
if(typeof v=="string"){
if(b){
a[a.length]=","}a.push(s.string(i),":",v);
b=true}}}a[a.length]="}";
return a.join("")}return"null"},string:function(x){
if(/["\\\x00-\x1f]/.test(x)){
x=x.replace(/([\x00-\x1f\\"])/g,function(a,b){
var c=m[b];
if(c){
return c}c=b.charCodeAt();
return"\\u00"+Math.floor(c/16).toString(16)+(c%16).toString(16)})}return'"'+x+'"'}};
SimileAjax.JSON.toJSONString=function(o){
if(o instanceof Object){
return s.object(o)}else{
if(o instanceof Array){
return s.array(o)}else{
return o.toString()}}};
SimileAjax.JSON.parseJSON=function(){
try{
return !(/[^,:{}\[\]0-9.\-+Eaeflnr-u \n\r\t]/.test(this.replace(/"(\\.|[^"\\])*"/g,"")))&&eval("("+this+")")}catch(e){
return false}}})();
...
D+=C.substring(F,B+2)}else{
D+=C.substring(F,B)+A[E].toString()}F=B+2}}}if(F<C.length){
D+=C.substring(F)}return D};
...
if(typeof A=="string"){
A=A.toLowerCase()}return(A=="iso8601"||A=="iso 8601")?SimileAjax.DateTime.parseIso8601DateTime:SimileAjax.DateTime.parseGregorianDateTime};
...
if(typeof D=="object"){
B=D.getTime()}else{
B=Number(D)}if(typeof C=="object"){
A=C.getTime()}else{
A=Number(C)}return B-A};
...
_initialized:false,_listeners:[],_draggedElement:null,_draggedElementCallback:null,_dropTargetHighlightElement:null,_lastCoords:null,_ghostCoords:null,_draggingMode:"",_dragging:false,_layers:[]};
SimileAjax.WindowManager.initialize=function(){
if(SimileAjax.WindowManager._initialized){
return}SimileAjax.DOM.registerEvent(document.body,"mousedown",SimileAjax.WindowManager._onBodyMouseDown);
...
SimileAjax.WindowManager._handleMouseDown(E,D,C)},A)};
...
return true}for(var A=B.index+1;
A<SimileAjax.WindowManager._layers.length;
A++){
...
break}}E--}SimileAjax.WindowManager._popToLayer(E)};
...
if(A.keyCode==27){
SimileAjax.WindowManager._cancelDragging()}else{
...
try{
if("onDragStart" in L){
L.onDragStart()}if("ghost" in L&&L.ghost){
...
L._ghostElmt=K}SimileAjax.WindowManager._dragging=true;
SimileAjax.WindowManager._lastCoords={
x:M.clientX,y:M.clientY};
document.body.focus()}catch(H){
...
x:M.clientX,y:M.clientY};
if("onDragBy" in L){
L.onDragBy(J,I)}if("_ghostElmt" in L){
var K=L._ghostElmt;
SimileAjax.WindowManager._ghostCoords.left+=J;
SimileAjax.WindowManager._ghostCoords.top+=I;
K.style.left=SimileAjax.WindowManager._ghostCoords.left+"px";
...
O.style.top=SimileAjax.WindowManager._ghostCoords.top+"px"}if("droppable" in L&&L.droppable){
...
SimileAjax.WindowManager._potentialDropTarget=null}var A=false;
if(B!=null){
...
SimileAjax.WindowManager._dropTargetHighlightElement=F}}}}}catch(H){
SimileAjax.Debug.exception("WindowManager: Error handling mouse move",H);
SimileAjax.WindowManager._cancelDragging()}}SimileAjax.DOM.cancelEvent(M);
return false}};
...
if("onDragEnd" in C){
C.onDragEnd()}if("droppable" in C&&C.droppable){
var D=false;
...
if("onDropOn" in C){
C.onDropOn(E)}E.ondrop(SimileAjax.WindowManager._draggedElement,SimileAjax.WindowManager._draggingMode);
D=true}}if(!D){
}}}}finally{
SimileAjax.WindowManager._cancelDragging()}SimileAjax.DOM.cancelEvent(A);
return false}};
...
delete A._ghostElmt}if(SimileAjax.WindowManager._dropTargetHighlightElement!=null){
document.body.removeChild(SimileAjax.WindowManager._dropTargetHighlightElement);
SimileAjax.WindowManager._dropTargetHighlightElement=null}if(SimileAjax.WindowManager._draggingModeIndicatorElmt!=null){
document.body.removeChild(SimileAjax.WindowManager._draggingModeIndicatorElmt);
...
B(A)}}else{
if(D){
D(A.statusText,A.status,A)}}}catch(C){
SimileAjax.Debug.exception("XmlHttp: Error handling onReadyStateChange",C)}break}};
...
SimileAjax.DOM.registerEventWithObject(this._div,"DOMMouseScroll",this,"_onMouseScroll")}else{
SimileAjax.DOM.registerEventWithObject(this._div,"mousewheel",this,"_onMouseScroll")}}this._innerDiv=this._timeline.getDocument().createElement("div");
...
this._eventListener={
onAddMany:function(){
A._onAddMany()},onClear:function(){
A._onClear()}};
this._eventSource.addListener(this._eventListener)}this._eventPainter=G.eventPainter;
...
this._eventSource=null}this._timeline=null;
this._bandInfo=null;
this._labeller=null;
this._ether=null;
...
this._syncWithBand.removeOnScrollListener(this._syncWithBandHandler)}this._syncWithBand=B;
...
return}var C=this._eventPainter.getType()=="overview";
...
if(this._timeline.isHorizontal()){
return this._div.offsetHeight}else{
...
if(SimileAjax.Platform.browser.isIE){
B.style.cursor="move"}else{
B.style.cursor="-moz-grab"}D.appendChild(B);
return B};
...
this.setCenterVisibleDate(this.pixelOffsetToDate(B+this._viewLength))}else{
if(B>3*this._viewLength/2){
this.setCenterVisibleDate(this.pixelOffsetToDate(B-this._viewLength))}}this._autoScroll(Math.round(this._viewLength/2-this._ether.dateToPixelOffset(A)),C)};
...
var I=0;
if(H.wheelDelta){
I=H.wheelDelta/120}else{
if(H.detail){
I=-H.detail/3}}var F=this._theme.mouseWheel;
...
if(I>0){
D=true}if(I<0){
D=false}this._timeline.zoom(D,E.x,E.y,G)}}else{
if(F==="scroll"){
var C=50*(I<0?-1:1);
this._moveEther(C)}}}if(H.stopPropagation){
H.stopPropagation()}H.cancelBubble=true;
if(H.preventDefault){
H.preventDefault()}H.returnValue=false};
...
this._moveEther(this._scrollSpeed);
break;
default:return true}this.closeBubble();
SimileAjax.DOM.cancelEvent(A);
return false}return true};
...
A._moveEther(F)},0,D,1000,C);
B.run()};
Timeline._Band.prototype._moveEther=function(A){
this.closeBubble();
if(!this._timeline.shiftOK(this._index,A)){
return}this._viewOffset+=A;
this._ether.shiftPixels(-A);
if(this._timeline.isHorizontal()){
this._div.style.left=this._viewOffset+"px"}else{
this._div.style.top=this._viewOffset+"px"}if(this._viewOffset>-this._viewLength*0.5||this._viewOffset<-this._viewLength*(Timeline._Band.SCROLL_MULTIPLES-1.5)){
this._recenterDiv()}else{
this.softLayout()}this._onChanging()};
...
this._div.style.width=(Timeline._Band.SCROLL_MULTIPLES*this._viewLength)+"px"}else{
...
G.compositeIconHeight=this._params.compositeIconHeight||G.customIconHeight}else{
G.compositeIcon=G.defaultIcon;
G.compositeIconWidth=G.defaultIconWidth;
G.compositeIconHeight=G.defaultIconHeight}G.defaultStackIcon="icon" in this._params.stackConcurrentPreciseInstantEvents?this._params.stackConcurrentPreciseInstantEvents.icon:G.defaultIcon;
...
this.paintEvent(E,G,this._params.theme,Q(E))}else{
if(A!=null&&A.getStart().getTime()==E.getStart().getTime()){
M[M.length-1].push(E)}else{
M.push([E]);
A=E}}}}for(var J=0;
J<M.length;
J++){
var K=M[J];
if(K.length==1){
this.paintEvent(K[0],G,this._params.theme,Q(E))}else{
var C=-1;
for(var I=0;
C<0&&I<K.length;
I++){
C=Q(K[I])}if(H){
this.paintStackedPreciseInstantEvents(K,G,this._params.theme,C)}else{
this.paintCompositePreciseInstantEvents(K,G,this._params.theme,C)}}}}else{
while(F.hasNext()){
var E=F.next();
if(R(E)){
this.paintEvent(E,G,this._params.theme,Q(E))}}}this._highlightLayer.style.display="block";
...
this._frc=SimileAjax.Graphics.getFontRenderingContext(A)}this._frc.update();
this._tracks=[];
if(this._highlightLayer!=null){
B.removeLayerDiv(this._highlightLayer)}this._highlightLayer=B.createLayerDiv(105,"timeline-band-highlights");
this._highlightLayer.style.display="none";
if(this._lineLayer!=null){
B.removeLayerDiv(this._lineLayer)}this._lineLayer=B.createLayerDiv(110,"timeline-band-lines");
this._lineLayer.style.display="none";
if(this._eventLayer!=null){
B.removeLayerDiv(this._eventLayer)}this._eventLayer=B.createLayerDiv(115,"timeline-band-events");
this._eventLayer.style.display="none"};
Timeline.CompactEventPainter.prototype.paintEvent=function(B,C,D,A){
if(B.isInstant()){
this.paintInstantEvent(B,C,D,A)}else{
this.paintDurationEvent(B,C,D,A)}};
Timeline.CompactEventPainter.prototype.paintInstantEvent=function(B,C,D,A){
if(B.isImprecise()){
this.paintImpreciseInstantEvent(B,C,D,A)}else{
this.paintPreciseInstantEvent(B,C,D,A)}};
Timeline.CompactEventPainter.prototype.paintDurationEvent=function(B,C,D,A){
if(B.isImprecise()){
this.paintImpreciseDurationEvent(B,C,D,A)}else{
...
E.className="timeline-event-icon-default"}else{
E.width=H.getProperty("iconWidth")||F.customIconWidth;
E.height=H.getProperty("iconHeight")||F.customIconHeight}var J={
text:H.getText(),color:H.getTextColor()||H.getColor(),className:H.getClassName()};
...
C<J.length;
C++){
A.push(J[C].getProperty("tooltip")||J[C].getText())}var E={
tooltip:A.join("; ")};
var G={
url:H.compositeIcon,width:H.compositeIconWidth,height:H.compositeIconHeight,className:"timeline-event-icon-composite"};
var M={
...
r.className="timeline-event-icon-stack timeline-event-icon-default"}else{
r.width=i.getProperty("iconWidth")||j.customIconWidth;
r.height=i.getProperty("iconHeight")||j.customIconHeight;
r.className="timeline-event-icon-stack"}return r};
var C=O(T[0]);
var V=5;
var D=0;
var g=0;
...
var K={
text:a,iconData:X,labelSize:W,iconLeft:C.width+n*V-X.width};
...
n++){
Y.push({
start:D,end:M})}var f=Math.ceil(p/j.trackHeight);
for(var n=0;
n<f;
n++){
var P=Y[n];
P.end=Math.max(P.end,g)}var k=this._fitTracks(F,Y);
...
var v=u[s];
if(s==w){
v.style.zIndex=u.length}else{
v.style.zIndex=u.length-s}}}catch(t){
}};
var d=function(u){
var w=l[u];
var r=T[u];
...
tooltip:i},{
text:w.text},F+w.labelLeft,Z+w.top,w.labelSize.width,w.labelSize.height,c);
...
tooltip:e},{
text:e},F+J,Z+m,H.width,H.height,c);
...
var E={
start:I.getStart(),end:I.getEnd(),latestStart:I.getLatestStart(),earliestEnd:I.getEarliestEnd(),isInstant:true};
var F={
url:I.getIcon()};
if(F.url==null){
F=null}else{
F.width=I.getProperty("iconWidth")||G.customIconWidth;
F.height=I.getProperty("iconHeight")||G.customIconHeight}var K={
text:I.getText(),color:I.getTextColor()||I.getColor(),className:I.getClassName()};
var H=this.paintTapeIconLabel(I.getStart(),C,E,F,K,G,B,A);
var J=this;
var D=F!=null?function(M,L,N){
return J._onClickInstantEvent(H.iconElmtData.elmt,L,I)}:function(M,L,N){
...
this._eventIdToElmt[I.getID()]=H.iconElmtData.elmt}else{
...
tooltip:I.getProperty("tooltip")||I.getText()};
var E={
start:I.getStart(),end:I.getEnd(),isInstant:false};
var F={
url:I.getIcon()};
if(F.url==null){
F=null}else{
...
tooltip:I.getProperty("tooltip")||I.getText()};
var E={
start:I.getStart(),end:I.getEnd(),latestStart:I.getLatestStart(),earliestEnd:I.getEarliestEnd(),isInstant:false};
var F={
url:I.getIcon()};
if(F.url==null){
F=null}else{
...
Q++){
W.push({
start:L,end:A})}C=X.trackHeight-(b%X.tapeHeight)}var N=0;
var U=0;
if(I!=null){
if("iconAlign" in I&&I.iconAlign=="center"){
N=-Math.floor(I.width/2)}U=N+I.width+X.iconLabelGap;
if(B>0){
W[B-1].end=Math.max(W[B-1].end,U)}var E=I.height+X.iconBottomMargin+C;
while(E>0){
W.push({
start:N,end:U});
E-=X.trackHeight}}var P=a.text;
var H=this._frc.computeSize(P);
var M=H.height+X.labelBottomMargin+C;
var J=U+H.width+X.labelRightMargin;
if(B>0){
W[B-1].end=Math.max(W[B-1].end,J)}for(var Y=0;
M>0;
Y++){
if(B+Y<W.length){
var T=W[B+Y];
T.end=J}else{
W.push({
start:0,end:J})}M-=X.trackHeight}var G=this._fitTracks(d,W);
...
D.impreciseTapeElmtData=this._paintEventTape(O,S,X.tapeHeight,K,F(S.start),F(S.end),c.event.duration.impreciseColor,c.event.duration.impreciseOpacity,X,c)}if(!S.isInstant&&"start" in S&&"end" in S){
D.tapeElmtData=this._paintEventTape(O,S,X.tapeHeight,K,d,F("earliestEnd" in S?S.earliestEnd:S.end),S.color,100,X,c)}}if(I!=null){
D.iconElmtData=this._paintEventIcon(O,I,K+b,d+N,X,c)}return D};
...
D<C.length;
D++){
this._tracks[H+D]=F+C[D].end}return H};
...
A.title=C.tooltip}this._eventLayer.appendChild(A);
return{
left:G,top:H,width:E.iconWidth,height:E.iconHeight,elmt:A}};
...
B.title=E.tooltip}if("color" in I&&typeof I.color=="string"){
B.style.color=I.color}if("className" in I&&typeof I.className=="string"){
B.className+=" "+I.className}this._eventLayer.appendChild(B);
return{
left:C,top:F,width:A,height:G,elmt:B}};
...
L.title=G.tooltip}if(E!=null&&typeof H.color=="string"){
L.style.backgroundColor=E}if("backgroundImage" in H&&typeof H.backgroundImage=="string"){
L.style.backgroundImage="url("+backgroundImage+")";
L.style.backgroundRepeat=("backgroundRepeat" in H&&typeof H.backgroundRepeat=="string")?H.backgroundRepeat:"repeat"}SimileAjax.Graphics.setOpacity(L,C);
if("className" in H&&typeof H.className=="string"){
L.className+=" "+H.className}this._eventLayer.appendChild(L);
return{
left:D,top:J,width:B,height:K,elmt:L}};
...
C<B.length;
C++){
D.push(B[C].getID())}this._fireOnSelect(D);
A.cancelBubble=true;
SimileAjax.DOM.cancelEvent(A);
return false};
...
var E=B.offsetY+D.top}this._showBubble(A,E,[C]);
...
var C=document.createElement("div");
E.appendChild(C);
B[D].fillInfoBubble(C,this._params.theme,this._band.getLabeller())}SimileAjax.WindowManager.cancelPopups();
...
this._band.removeLayerDiv(this._layerDiv)}this._layerDiv=this._band.createLayerDiv(this._zIndex);
...
if(this._cssClass){
B.className+=" "+this._cssClass}if(this._color!=null){
B.style.backgroundColor=this._color}if(this._opacity<100){
SimileAjax.Graphics.setOpacity(B,this._opacity)}this._layerDiv.appendChild(B);
var J=K();
J.className="timeline-highlight-label timeline-highlight-label-start";
var C=J.rows[0].cells[0];
C.innerHTML=this._startLabel;
if(this._cssClass){
C.className="label_"+this._cssClass}this._layerDiv.appendChild(J);
var G=K();
G.className="timeline-highlight-label timeline-highlight-label-end";
var D=G.rows[0].cells[0];
D.innerHTML=this._endLabel;
if(this._cssClass){
D.className="label_"+this._cssClass}this._layerDiv.appendChild(G);
if(this._timeline.isHorizontal()){
B.style.left=F+"px";
...
G.style.left=I+"px";
G.style.width=(this._endLabel.length)+"em"}else{
B.style.top=F+"px";
B.style.height=(I-F)+"px";
J.style.bottom=F+"px";
J.style.height="1.5px";
G.style.top=I+"px";
G.style.height="1.5px"}}this._layerDiv.style.display="block"};
...
if(this._layerDiv!=null){
this._band.removeLayerDiv(this._layerDiv)}this._layerDiv=this._band.createLayerDiv(10);
this._layerDiv.setAttribute("name","span-highlight-decorator");
this._layerDiv.style.display="none";
...
if(this._color!=null){
F.style.backgroundColor=this._color}if(this._opacity<100){
SimileAjax.Graphics.setOpacity(F,this._opacity)}this._layerDiv.appendChild(F);
if(this._timeline.isHorizontal()){
F.style.left=B+"px";
F.style.width=this._width}else{
F.style.top=B+"px";
F.style.height=this._width}}this._layerDiv.style.display="block"};
...
var F={
trackOffset:Math.round(this._band.getViewWidth()/2-G/2),trackHeight:G,trackGap:I.track.gap,trackIncrement:G+I.track.gap,icon:I.instant.icon,iconWidth:I.instant.iconWidth,iconHeight:I.instant.iconHeight,labelWidth:I.label.width};
...
var H=E.next();
if(J(H)){
this.paintEvent(H,F,this._params.theme,A(H))}}this._highlightLayer.style.display="block";
this._lineLayer.style.display="block";
this._eventLayer.style.display="block";
...
this._lineLayer.style.display="none";
if(this._eventLayer!=null){
B.removeLayerDiv(this._eventLayer)}this._eventLayer=B.createLayerDiv(110,"timeline-band-events");
this._eventLayer.style.display="none"};
...
this._paintEventLine(L,H,F,O,P,S)}var N=Math.round(P.trackOffset+O*P.trackIncrement+P.trackHeight/2-E.height/2);
...
this._paintEventLine(O,K,H,R,S,W)}var Q=Math.round(S.trackOffset+R*S.trackIncrement+S.trackHeight/2-G.height/2);
...
return C}if(C<this._upperTracks.length){
...
this._upperTracks.push({
solid:Number.POSITIVE_INFINITY,text:Number.POSITIVE_INFINITY,line:Number.POSITIVE_INFINITY})}for(var D=F;
D<E;
D++){
I(this._upperTracks[D])}}else{
if(E==this._lowerTracks.length){
this._lowerTracks.push({
solid:Number.POSITIVE_INFINITY,text:Number.POSITIVE_INFINITY,line:Number.POSITIVE_INFINITY})}for(var D=F;
D<E;
D++){
I(this._lowerTracks[D])}}return H};
...
if(D>A){
B.style.top=(H-I)+"px";
B.style.borderTop=C}else{
B.style.top=H+"px";
B.style.borderBottom=C}B.style.borderLeft=C;
...
if(J._title!=null){
A.title=J._title}this._eventLayer.appendChild(A);
return{
left:C,top:I,width:F.iconWidth,height:F.iconHeight,elmt:A}};
...
if(I._title!=null){
B.title=I._title}var D=I.getTextColor();
if(D==null){
D=I.getColor()}if(D!=null){
B.style.color=D}this._eventLayer.appendChild(B);
return{
left:C,top:F,width:A,height:G,elmt:B}};
...
if(L._title!=null){
M.title=L._title}SimileAjax.Graphics.setOpacity(M,C);
this._eventLayer.appendChild(M);
return{
left:D,top:J,width:F,height:E,elmt:M}};
...
this._band.removeLayerDiv(this._markerLayer)}this._markerLayer=this._band.createLayerDiv(100);
this._markerLayer.setAttribute("name","ether-markers");
this._markerLayer.style.display="none";
if(this._lineLayer){
this._band.removeLayerDiv(this._lineLayer)}this._lineLayer=this._band.createLayerDiv(1);
...
B(C)}this._markerLayer.style.display="block";
this._lineLayer.style.display="block"};
...
startTime:A.startTime,endTime:E,unit:A.unit,multiple:A.multiple});
D++;
A.startTime=E}if(B<A.endTime){
this._zones.splice(D,0,{
startTime:E,endTime:B,unit:C.unit,multiple:(C.multiple)?C.multiple:1});
D++;
A.startTime=B;
E=B}else{
...
break}D++}var E=this._zones.length-1;
while(E>=0){
...
J(K,G)}}this._markerLayer.style.display="block";
this._lineLayer.style.display="block"};
...
A(B)}this._markerLayer.style.display="block";
this._lineLayer.style.display="block"};
...
if(F!=0){
return{
text:"Q"+(F+1),emphasized:false}}else{
return{
text:"Y"+(G.getUTCFullYear()-C._startDate.getUTCFullYear()+1),emphasized:true}}}};
...
O.style.left=N+"px";
O.style.top="0px"}}else{
this.positionDiv=function(O,N){
O.style.left=N+"px";
O.style.bottom="0px"}}}else{
if(E=="Left"){
this.positionDiv=function(O,N){
O.style.top=N+"px";
O.style.left="0px"}}else{
this.positionDiv=function(O,N){
O.style.top=N+"px";
O.style.right="0px"}}}var D=C.ether.interval.marker;
...
if(K.opacity<100){
SimileAjax.Graphics.setOpacity(V,K.opacity)}if(A){
V.style.left=U+"px"}else{
V.style.top=U+"px"}P.appendChild(V)}if(a==SimileAjax.DateTime.WEEK){
...
if(B.opacity<100){
SimileAjax.Graphics.setOpacity(X,B.opacity)}if(A){
X.style.left=Q+"px";
X.style.width=W+"px"}else{
X.style.top=Q+"px";
X.style.height=W+"px"}P.appendChild(X)}var Z=c.labelInterval(T,a);
var O=I.getDocument().createElement("div");
O.innerHTML=Z.text;
O.className="timeline-date-label";
if(Z.emphasized){
O.className+=" timeline-date-label-em"}this.positionDiv(O,U);
Y.appendChild(O);
return O}};
...
if(F<100){
SimileAjax.Graphics.setOpacity(this._highlightDiv,F)}C.appendChild(this._highlightDiv)}};
...
this._highlightDiv.style.height=(E.getViewWidth()-4)+"px"}else{
...
this._start=this._unit.parseFromObject(this._params.startsOn)}else{
if("endsOn" in this._params){
this._start=this._unit.parseFromObject(this._params.endsOn);
this.shiftPixels(-this._timeline.getPixelLength())}else{
if("centersOn" in this._params){
this._start=this._unit.parseFromObject(this._params.centersOn);
this.shiftPixels(-this._timeline.getPixelLength()/2)}else{
...
var C=B;
if(D&&(B>0)){
C=B-1}if(!D&&(B<(this._band._zoomSteps.length-1))){
C=B+1}this._band._zoomIndex=C;
...
startTime:A.startTime,endTime:F,magnify:A.magnify});
C++;
A.startTime=F}if(this._unit.compare(B,A.endTime)<0){
this._zones.splice(C,0,{
startTime:F,endTime:B,magnify:G.magnify*A.magnify});
C++;
A.startTime=B;
F=B}else{
A.magnify*=G.magnify;
F=A.endTime}}}}if("startsOn" in this._params){
this._start=this._unit.parseFromObject(this._params.startsOn)}else{
if("endsOn" in this._params){
this._start=this._unit.parseFromObject(this._params.endsOn);
this.shiftPixels(-this._timeline.getPixelLength())}else{
...
break}G++}while(this._unit.compare(I,B)<0){
var F=this._zones[G];
...
this._lastEventID=0}this._lastEventID+=1;
...
return{
band:null,evt:null}}var B=Timeline.getTimelineFromID(D[2]);
var E=B.getBand(D[3]);
...
if(B==null){
B=Timeline.GregorianDateLabeller.prototype.defaultLabelInterval}return B.call(this,A,C)};
...
break}case SimileAjax.DateTime.YEAR:case SimileAjax.DateTime.DECADE:case SimileAjax.DateTime.CENTURY:case SimileAjax.DateTime.MILLENNIUM:var E=B.getUTCFullYear();
if(E>0){
D=B.getUTCFullYear()}else{
D=(1-E)+"BC"}F=(C==SimileAjax.DateTime.MONTH)||(C==SimileAjax.DateTime.DECADE&&E%100==0)||(C==SimileAjax.DateTime.CENTURY&&E%1000==0);
break;
default:D=B.toUTCString()}return{
text:D,emphasized:F}};
...
var F={
trackOffset:I.track.offset,trackHeight:G,trackGap:I.track.gap,trackIncrement:G+I.track.gap,icon:I.instant.icon,iconWidth:I.instant.iconWidth,iconHeight:I.instant.iconHeight,labelWidth:I.label.width,maxLabelChar:I.label.maxLabelChar,impreciseIconMargin:I.instant.impreciseIconMargin};
var D=this._band.getMinDate();
var B=this._band.getMaxDate();
var J=(this._filterMatcher!=null)?this._filterMatcher:function(K){
return true};
...
if(G!=null){
E.push(G)}this._fireEventPaintListeners("paintedEvent",N,E);
...
if(I!=null){
G.push(I)}this._fireEventPaintListeners("paintedEvent",P,G);
...
if(F!=null){
D.push(F)}this._fireEventPaintListeners("paintedEvent",M,D);
this._eventIdToElmt[M.getID()]=B.elmt;
this._tracks[O]=H};
...
if(H!=null){
D.push(H)}this._fireEventPaintListeners("paintedEvent",O,D);
...
var J;
if(D>0){
J=G.trackOffset+B*G.trackIncrement+D+G.impreciseIconMargin}else{
var H=G.trackOffset+B*G.trackIncrement+G.trackHeight/2;
J=Math.round(H-G.iconHeight/2)}var E=SimileAjax.Graphics.createTranslucentImage(I);
var A=this._timeline.getDocument().createElement("div");
...
A.appendChild(E);
if(K._title!=null){
A.title=K._title}this._eventLayer.appendChild(A);
return{
left:C,top:J,width:G.iconWidth,height:G.iconHeight,elmt:A}};
...
B.innerHTML=L;
if(K._title!=null){
B.title=K._title}var F=K.getTextColor();
if(F==null){
F=K.getColor()}if(F!=null){
B.style.color=F}if(G.event.highlightLabelBackground&&C>=0){
B.style.background=this._getHighlightColor(C,G)}this._eventLayer.appendChild(B);
return{
left:D,top:H,width:A,height:J,elmt:B}};
...
O.style.top=K+"px";
if(N._title!=null){
O.title=N._title}if(G!=null){
O.style.backgroundColor=G}var L=N.getTapeImage();
var H=N.getTapeRepeat();
H=H!=null?H:"repeat";
if(L!=null){
O.style.backgroundImage="url("+L+")";
O.style.backgroundRepeat=H}SimileAjax.Graphics.setOpacity(O,C);
this._eventLayer.appendChild(O);
return{
left:D,top:K,width:F,height:E,elmt:O}};
...
this._highlightLayer.appendChild(G)}return G};
...
var C=this._band.getEventSource();
if(C==null){
return}this._prepareForPainting();
var H=this._params.theme.event;
var F={
trackOffset:H.overviewTrack.offset,trackHeight:H.overviewTrack.height,trackGap:H.overviewTrack.gap,trackIncrement:H.overviewTrack.height+H.overviewTrack.gap};
var D=this._band.getMinDate();
var B=this._band.getMaxDate();
...
var G=E.next();
if(I(G)){
this.paintEvent(G,F,this._params.theme,A(G))}}this._highlightLayer.style.display="block";
this._eventLayer.style.display="block";
this._band.updateEventTrackInfo(this._tracks.length,F.trackIncrement)};
...
this._tracks=[];
if(this._highlightLayer!=null){
A.removeLayerDiv(this._highlightLayer)}this._highlightLayer=A.createLayerDiv(105,"timeline-band-highlights");
this._highlightLayer.style.display="none";
if(this._eventLayer!=null){
A.removeLayerDiv(this._eventLayer)}this._eventLayer=A.createLayerDiv(110,"timeline-band-events");
this._eventLayer.style.display="none"};
...
if(D){
C=null}else{
C=C!=null?C:E.event.duration.color}var G=this._paintEventTick(I,B,C,100,H,E);
...
F++){
if(C<this._tracks[F]){
break}}this._tracks[F]=C;
var E=L.getColor(),G=L.getClassName();
if(G){
E=null}else{
E=E!=null?E:H.event.duration.color}var D=this._paintEventTape(L,F,J,C,E,100,K,H,G);
...
if(F){
M.className+=" small-"+F}M.style.left=D+"px";
M.style.width=A+"px";
M.style.top=I+"px";
M.style.height=J+"px";
if(E){
M.style.backgroundColor=E}if(C<100){
SimileAjax.Graphics.setOpacity(M,C)}this._eventLayer.appendChild(M);
return{
left:D,top:I,width:A,height:J,elmt:M}};
...
if(E){
K.className+=" small-"+E}if(B<100){
SimileAjax.Graphics.setOpacity(K,B)}this._eventLayer.appendChild(K);
return{
left:C,top:H,width:A,height:I,elmt:K}};
...
L=D.firstChild.nodeValue}var B=(D.getAttribute("isDuration")===null&&D.getAttribute("durationEvent")===null)||D.getAttribute("isDuration")=="false"||D.getAttribute("durationEvent")=="false";
...
D[A.getAttribute("name")]=A.firstChild.firstChild.nodeValue}A=A.nextSibling}if(D.start==null&&D.date!=null){
D.start=D.date}var C=(D.isDuration===null&&D.durationEvent===null)||D.isDuration=="false"||D.durationEvent=="false";
...
B<A.length;
B++){
this._events.add(A[B])}this._fire("onAddMany",[])};
...
return A}else{
if(A.substr(0,1)=="/"){
return B.substr(0,B.indexOf("/",B.indexOf("://")+3))+A}else{
return B+A}}}};
Timeline.DefaultEventSource.Event=function(A){
function D(E){
return(A[E]!=null&&A[E]!="")?A[E]:null}var C=A.id?A.id.trim():"";
...
B.push("start is > latestStart")}if(this._start>this._earliestEnd){
this._earliestEnd=this._latestStart;
B.push("start is > earliestEnd")}if(this._start>this._end){
this._end=this._earliestEnd;
B.push("start is > end")}if(this._latestStart>this._earliestEnd){
this._earliestEnd=this._latestStart;
B.push("latestStart is > earliestEnd")}if(this._latestStart>this._end){
this._end=this._earliestEnd;
B.push("latestStart is > end")}if(this._earliestEnd>this._end){
this._end=this._earliestEnd;
B.push("earliestEnd is > end")}this._eventID=D("eventID");
this._text=(A.text!=null)?SimileAjax.HTML.deEntify(A.text):"";
if(B.length>0){
this._text+=" PROBLEM: "+B.join(", ")}this._description=SimileAjax.HTML.deEntify(A.description);
this._image=D("image");
this._link=D("link");
...
this._trackNum=parseInt(this._trackNum)}this._wikiURL=null;
this._wikiSection=null};
Timeline.DefaultEventSource.Event.prototype={
getID:function(){
return this._id},isInstant:function(){
return this._instant},isImprecise:function(){
return this._start!=this._latestStart||this._end!=this._earliestEnd},getStart:function(){
return this._start},getEnd:function(){
return this._end},getLatestStart:function(){
return this._latestStart},getEarliestEnd:function(){
return this._earliestEnd},getEventID:function(){
return this._eventID},getText:function(){
return this._text},getDescription:function(){
return this._description},getImage:function(){
return this._image},getLink:function(){
return this._link},getIcon:function(){
return this._icon},getColor:function(){
return this._color},getTextColor:function(){
return this._textColor},getClassName:function(){
return this._classname},getTapeImage:function(){
return this._tapeImage},getTapeRepeat:function(){
return this._tapeRepeat},getTrackNum:function(){
return this._trackNum},getProperty:function(A){
return null},getWikiURL:function(){
return this._wikiURL},getWikiSection:function(){
return this._wikiSection},setWikiInfo:function(B,A){
this._wikiURL=B;
this._wikiSection=A},fillDescription:function(A){
A.innerHTML=this._description},fillWikiInfo:function(D){
D.style.display="none";
if(this._wikiURL==null||this._wikiSection==null){
return}var C=this.getProperty("wikiID");
if(C==null||C.length==0){
C=this.getText()}if(C==null||C.length==0){
...
D.appendChild(A);
D.appendChild(document.createTextNode("]"))},fillTime:function(A,B){
if(this._instant){
if(this.isImprecise()){
A.appendChild(A.ownerDocument.createTextNode(B.labelPrecise(this._start)));
A.appendChild(A.ownerDocument.createElement("br"));
A.appendChild(A.ownerDocument.createTextNode(B.labelPrecise(this._end)))}else{
A.appendChild(A.ownerDocument.createTextNode(B.labelPrecise(this._start)))}}else{
if(this.isImprecise()){
A.appendChild(A.ownerDocument.createTextNode(B.labelPrecise(this._start)+" ~ "+B.labelPrecise(this._latestStart)));
A.appendChild(A.ownerDocument.createElement("br"));
A.appendChild(A.ownerDocument.createTextNode(B.labelPrecise(this._earliestEnd)+" ~ "+B.labelPrecise(this._end)))}else{
A.appendChild(A.ownerDocument.createTextNode(B.labelPrecise(this._start)));
A.appendChild(A.ownerDocument.createElement("br"));
A.appendChild(A.ownerDocument.createTextNode(B.labelPrecise(this._end)))}}},fillInfoBubble:function(A,E,M){
...
A.appendChild(D)}var L=K.createElement("div");
var C=K.createTextNode(J);
if(H!=null){
var I=K.createElement("a");
I.href=H;
I.appendChild(C);
L.appendChild(I)}else{
L.appendChild(C)}E.event.bubble.titleStyler(L);
A.appendChild(L);
...
if(B==null){
B=Timeline.getDefaultLocale()}var A=Timeline.ClassicTheme.implementations[B];
if(A==null){
A=Timeline.ClassicTheme._Impl}return new A()};
...
line:{
show:true,opacity:25},weekend:{
opacity:30},marker:{
hAlign:"Bottom",vAlign:"Right"}}};
this.event={
track:{
height:10,gap:2,offset:2,autoWidthMargin:1.5},overviewTrack:{
offset:20,tickHeight:6,height:2,gap:1,autoWidthMargin:5},tape:{
height:4},instant:{
icon:Timeline.urlPrefix+"images/dull-blue-circle.png",iconWidth:10,iconHeight:10,impreciseOpacity:20,impreciseIconMargin:3},duration:{
impreciseOpacity:20},label:{
backgroundOpacity:50,offsetFromLine:3},highlightColors:["#FFFF00","#FFC000","#FF0000","#0000FF"],highlightLabelBackground:false,bubble:{
width:250,maxHeight:0,titleStyler:function(A){
A.className="timeline-event-bubble-title"},bodyStyler:function(A){
A.className="timeline-event-bubble-body"},imageStyler:function(A){
A.className="timeline-event-bubble-image"},wikiStyler:function(A){
A.className="timeline-event-bubble-wiki"},timeStyler:function(A){
...
Timeline.timelines=[]}var B=Timeline.timelines.length;
...
centersOn:("date" in F)?F.date:new Date(),interval:SimileAjax.DateTime.gregorianUnitLengths[F.intervalUnit],pixelsPerInterval:F.intervalPixels,theme:G});
var C=new Timeline.GregorianEtherPainter({
unit:F.intervalUnit,multiple:("multiple" in F)?F.multiple:1,theme:G,align:("align" in F)?F.align:undefined});
var I={
showText:("showEventText" in F)?F.showEventText:true,theme:G};
if("eventPainterParams" in F){
for(var A in F.eventPainterParams){
I[A]=F.eventPainterParams[A]}}if("trackHeight" in F){
I.trackHeight=F.trackHeight}if("trackGap" in F){
I.trackGap=F.trackGap}var B=("overview" in F&&F.overview)?"overview":("layout" in F?F.layout:"original");
var E;
if("eventPainter" in F){
E=new F.eventPainter(I)}else{
switch(B){
case"overview":E=new Timeline.OverviewEventPainter(I);
break;
case"detailed":E=new Timeline.DetailedEventPainter(I);
break;
default:E=new Timeline.OriginalEventPainter(I)}}return{
width:F.width,eventSource:D,timeZone:("timeZone" in F)?F.timeZone:0,ether:H,etherPainter:C,eventPainter:E,theme:G,zoomIndex:("zoomIndex" in F)?F.zoomIndex:0,zoomSteps:("zoomSteps" in F)?F.zoomSteps:null}};
Timeline.createHotZoneBandInfo=function(F){
var G=("theme" in F)?F.theme:Timeline.getDefaultTheme();
var D=("eventSource" in F)?F.eventSource:null;
var H=new Timeline.HotZoneEther({
centersOn:("date" in F)?F.date:new Date(),interval:SimileAjax.DateTime.gregorianUnitLengths[F.intervalUnit],pixelsPerInterval:F.intervalPixels,zones:F.zones,theme:G});
var C=new Timeline.HotZoneGregorianEtherPainter({
unit:F.intervalUnit,zones:F.zones,theme:G,align:("align" in F)?F.align:undefined});
var I={
showText:("showEventText" in F)?F.showEventText:true,theme:G};
if("eventPainterParams" in F){
for(var A in F.eventPainterParams){
...
Timeline._defaultTheme=Timeline.ClassicTheme.create(Timeline.getDefaultLocale())}return Timeline._defaultTheme};
...
A++){
this._bands[A].dispose()}this._bands=null;
...
F.load(G.responseStream)}D(F,B)}finally{
A.hideLoadingMessage()}};
this.showLoadingMessage();
window.setTimeout(function(){
SimileAjax.XmlHttp.get(B,E,C)},0)};
...
var fDone=function(xmlhttp){
try{
f(eval("("+xmlhttp.responseText+")"),url)}finally{
tl.hideLoadingMessage()}};
this.showLoadingMessage();
window.setTimeout(function(){
SimileAjax.XmlHttp.get(url,fError,fDone)},0)};
...
H+=A._bandInfos[I].width}if(H>G||C){
D=H;
E();
A._distributeWidths()}}if(!A.autoWidth){
return}F()};
...
while(H.firstChild){
H.removeChild(H.firstChild)}var A=SimileAjax.Graphics.createTranslucentImage("http://static.simile.mit.edu/timeline/api/images/copyright-vertical.png");
...
this._bands.push(G)}this._distributeWidths();
for(var B=0;
B<this._bandInfos.length;
B++){
var F=this._bandInfos[B];
if("syncWith" in F){
this._bands[B].setSyncWithBand(this._bands[F.syncWith],("highlight" in F)?F.highlight:false)}}if(this.autoWidth){
for(var B=0;
B<this._bands.length;
B++){
this._bands[B].addOnScrollListener(this._autoWidthScrollListener)}}var D=SimileAjax.Graphics.createMessageBubble(E);
D.containerDiv.className="timeline-message-container";
H.appendChild(D.containerDiv);
D.contentDiv.className="timeline-message";
D.contentDiv.innerHTML="<img src='"+Timeline.urlPrefix+"Loading...";
...
D=Math.round(A*B/100)}else{
D=parseInt(E)}}else{
D=E}J.setBandShiftAndWidth(C,D);
J.setViewLength(G);
C+=D}};
Timeline._Impl.prototype.shiftOK=function(C,B){
var F=B>0,A=B<0;
if((F&&this.timeline_start==null)||(A&&this.timeline_stop==null)||(B==0)){
return(true)}var H=false;
for(var E=0;
E<this._bands.length&&!H;
E++){
H=this._bands[E].busy()}if(H){
return(true)}if((F&&this.timeline_at_start)||(A&&this.timeline_at_stop)){
return(false)}var D=false;
for(var E=0;
E<this._bands.length&&!D;
E++){
var G=this._bands[E];
if(F){
D=(E==C?G.getMinVisibleDateAfterDelta(B):G.getMinVisibleDate())>=this.timeline_start}else{
D=(E==C?G.getMaxVisibleDateAfterDelta(B):G.getMaxVisibleDate())<=this.timeline_stop}}if(F){
this.timeline_at_start=!D;
this.timeline_at_stop=false}else{
this.timeline_at_stop=!D;
this.timeline_at_start=false}return(D)};
Timeline._Impl.prototype.zoom=function(D,A,G,F){
var C=new RegExp("^timeline-band-([0-9]+)$");
var E=null;
var B=C.exec(F.id);
if(B){
E=parseInt(B[1])}if(E!=null){
this._bands[E].zoom(D,A,G,F)}this.paint()};
...
if(typeof A=="string"){
A=A.toLowerCase()}return(A=="iso8601"||A=="iso 8601")?Timeline.DateTime.parseIso8601DateTime:Timeline.DateTime.parseGregorianDateTime};
