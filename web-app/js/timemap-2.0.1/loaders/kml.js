...
TimeMap.loaders.kml.parse = function(kmlnode) {
    ...
    function findNodeTime(n, data) {
        ...
        if (tspan.length) {
            ...
            //Custom for RAGE: add 5 time range so markers show up as color coded bars in timeline
            //var end = new Date(data.start); IE 8 does not support creating a date from ISO-8601 string
            //http://stackoverflow.com/questions/6900857/alertparseint09-shows-me-0-why
            var end = new Date(Date.UTC(
            		parseInt(data.start.substring(0,4), 10), //year
            		parseInt(data.start.substring(5,7), 10) - 1, //month
            		parseInt(data.start.substring(8,10), 10), //day
            		parseInt(data.start.substring(11,13), 10), //hour
            		parseInt(data.start.substring(14,16), 10) )); //minute
            end.setMinutes(end.getMinutes() + 5);
            //toISOString does not exist in older browsers, so add - thanks stackoverflow
    ...
    getNodeList(kmlnode, "Placemark").each(function() {
        ...
        // look for marker
        //RAGE - load markers last so event handler is on Marker for placemarks with multigeometries
