...
TimeMapItem = function(data, dataset) {
    ...
    placemarks.forEach(function(placemark) {
        ...
        //RAGE only add click event to Markers - click event on polygons is broken!!!
...
TimeMapItem.openInfoWindowBasic = function() {
    ...
    } else if (item.getType() == "array") {
    	//RAGE - support events with multiple geometries
    	var marker = placemark[placemark.length - 1]; //last placemark is Marker because of order loaded in kml.js
    	marker.setInfoBubble(html);
    	marker.openBubble();
        // deselect when window is closed
        item.closeHandler = marker.closeInfoBubble.addHandler(function() { 
            // deselect
            ds.timemap.setSelected(undefined);
            // kill self
            marker.closeInfoBubble.removeHandler(item.closeHandler);
        });
    } else {
